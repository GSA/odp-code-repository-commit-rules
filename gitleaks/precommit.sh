...
rules_url="https://raw.githubusercontent.com/GSA/odp-code-repository-commit-rules/${expected_version}/gitleaks/rules.toml"
...
if [ ! -f "$bin_path" ] || [ "$actual_version" != "$expected_version" ]; then
    curl -L -o "$bin_path" "${base_url}/${bin_name}"
    curl -L -o "${hooks_path}/rules.toml" "${rules_url}"
    echo "$expected_version" > "${hooks_path}/gitleaks_version"
fi
...
expected_sum=$(curl --silent -L "${base_url}/${bin_name}.sum")
